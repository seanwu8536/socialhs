...
<meta charset="utf-8">
    ...
    <script>
    	...
    	function send_go(frm){
    		frm.method = "post";
    		frm.action = "HobbyController?type=updateOk";
    		frm.submit();	
    	}
    	function moveMap(){
...
<body class="bg-light text-dark">
    ...
    <div class="container">
        <h1 class="display-4 text-center my-5">모집 글 수정하기</h1>
    </div>
    <form action = "#">
        <div class="row my-3">
            <div class="col ml-4">
        ...
        <div class="form-group mx-5">
            <label for="title_location">제목</label><br>
            <input type="text" class="form-control" id="title" name = "title" value = "${viewVO.title }">
        </div>
        <div class="row">
            <div class="col ml-5">
                <label for="req_member">모집인원</label>
                <input id="req_member" type="number" min=0 class="form-control" name = "req_member" value = "${viewVO.req_member }">
            </div>
            <div class="col mr-5">
                <label for="time">시간</label>
                <input id="time" type="text" class="form-control" placeholder="구체적인 시간을 필수적으로 입력해주세요." name = "time" value = "${viewVO.time }">
            </div>
        </div>
        ...
        <div class="form-group mx-5">
            ...
            <input type="text" class="form-control" id="place" name = "place" placeholder="구체적인 장소를 필수적으로 입력해주세요." value = "${viewVO.place }">
        </div>

        <div class="form-group mx-5">
            <label for="content">내용</label>
            <textarea class="form-control" id="content" name = "content" rows="50">${viewVO.content }</textarea>
        </div>
	...
	<script>
		...
		var firstY = ${viewVO.longitude};
		var firstX = ${viewVO.latitude};
		var mapOptions = {
		...
		var marker1 = new naver.maps.Marker({ //seoul
		    position: new naver.maps.LatLng(firstY, firstX),
		    map: map
		});
